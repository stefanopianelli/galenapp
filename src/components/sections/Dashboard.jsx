...
import { Package, AlertTriangle, Trash2, History, Pill } from 'lucide-react';
            ...
            <div key={log.id} className="flex justify-between items-center text-sm border-b border-slate-100 pb-2 last:border-0">
              <div>
                <div 
                  ...
                  onClick={() => substanceInInventory && handleShowSubstanceInInventory(substanceInInventory.id)}
                >
                  {log.substance}
                </div>
              ...
              <div className="text-right">
                <Badge type={log.type === 'CARICO' ? 'success' : 'warning'}>
                  {log.type === 'CARICO' ? '+' : '-'}{log.quantity} {log.unit}
                </Badge>
              </div>
            </div>
          );
